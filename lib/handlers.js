...
// Container for all the tokensmethods
handlers._tokens = {};
...
// Checks - post
// Requires data: protocol, url, method, successCodes, timeoutSeconds
// Optional data: none
        ...
        _data.read("users", userPhone, function (err, userData) {
          if (!err && userData) {
            ...
            if (userChecks.length < config.maxChecks) {
                ...
                if (!err) {
                  ...
                  _data.update("users", userPhone, userData, function (err) {
                    if (!err) {
                      ...
                      callback(500, {
                        Error: "Could not updae the user with the new check",
                      });
                    }
                  });
                } else {
              ...
              callback(400, {
                Error:
                  "The user already has the maximum of checks(" +
                  config.maxChecks +
                  ")",
              });
            }
                    ...
                    function (err, userData) {
                      if (!err && userData) {
                        ...
                        if (checkPosition > -1) {
                          ...
                          callback(500, {
                            Error:
                              "Could not find the user's object, so could not remove it ",
                          });
                        }
                      } else {
                        callback(500, {
